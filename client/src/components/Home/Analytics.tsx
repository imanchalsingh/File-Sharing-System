...
const BASE = "/api/analytics"; // change if your Express router prefix differs

async function fetchStats(period: string): Promise<AnalyticsData> {
  const fetchOrThrow = async (url: string) => {
    ...
    try {
      ...
      throw new Error(
        `Network error hitting ${url} — is the backend running and is a proxy configured? (${networkErr})`
      );
    }
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      throw new Error(`${url} returned ${res.status}: ${body.slice(0, 200)}`);
    }
    return res.json();
  };

  const [statsRes, typesRes] = await Promise.all([
    fetchOrThrow(`${BASE}/stats?period=${period}`),
    fetchOrThrow(`${BASE}/file-types?period=${period}`),
  ]);

  // ── dailyShares ──────────────────────────────────────────────────────────
  const dailyShares = (statsRes.sharesByDay ?? []).map(
    (d: { date: string; shares: number; uniqueUsers: number }) => ({
      date: new Date(d.date).toLocaleDateString("en-US", { weekday: "short" }),
      shares: d.shares,
      uniqueFiles: d.uniqueUsers,
    })
  );

  // ── topFiles ─────────────────────────────────────────────────────────────
  const topFiles = (statsRes.topFiles ?? []).map(
    (f: { fileName: string; shares: number; downloads?: number; views?: number }) => ({
      name:
  ...
  const totalSourceCount = rawSources.reduce((s, x) => s + x.count, 0);
  const shareSources = rawSources.map((s) => ({
    name: s._id,
    value:
  ...
  const totalHourlyShares = (statsRes.hourlyActivity ?? []).reduce(
    (s: number, h: { shares: number }) => s + h.shares,
    0
  );
  const hourlyActivity = (statsRes.hourlyActivity ?? []).map(
    (h: { _id: number; shares: number }) => ({
      hour: formatHour(h._id),
      shares: h.shares,
      avg: Math.round(totalHourlyShares / 24),
    })
  );

  // ── fileTypeDistribution ─────────────────────────────────────────────────
  const fileTypeDistribution = (typesRes.fileTypeDistribution ?? []).map(
    (t: { type: string; count: number; shares: number }) => ({
      type: t.type.charAt(0).toUpperCase() + t.type.slice(1),
      count: t.count,
      shares: t.shares,
      color: getFileTypeColor(t.type),
    })
  );

  // ── performanceMetrics ───────────────────────────────────────────────────
  const totalShares: number = statsRes.totalShares ?? 0;
  const uniqueFiles: number = statsRes.uniqueFiles ?? 0;
  const avgSharesPerFile =
  ...
  const directCopyRate = directCopySource?.value ?? 100;
  ...
  return {
    ...
    performanceMetrics: {
      avgSharesPerFile,
      peakHourShares,
      mobileShareRate: 30, // device tracking not in current backend
      directCopyRate,
    },
    recentActivity: [], // no dedicated "recent activity" endpoint yet
    totalShares,
    uniqueFiles,
    totalDownloads: statsRes.uniqueDownloads ?? 0,
    totalViews: statsRes.uniqueViews ?? 0,
    conversionRate: parseFloat(statsRes.conversionRate ?? "0"),
  };
}
...
const Analytics: React.FC = () => {
  ...
  const loadData = async (period: string) => {
    ...
    try {
      ...
      // Print the real reason so it's easy to diagnose in the browser console
      const msg: string = err?.message ?? String(err);
      console.error("[Analytics] Backend fetch failed:", msg);
      setError(msg);
      // Do NOT replace analyticsData — keep EMPTY_DATA so the page still renders
    } finally {
      setLoading(false);
    }
  };
  ...
  const { totalShares, uniqueFiles, totalDownloads, totalViews } = analyticsData;

  const stats = [
    {
      title: "Total Link Copies",
      value: totalShares,
      change: `${analyticsData.performanceMetrics.avgSharesPerFile} avg / file`,
      icon: <Share2 className="w-6 h-6" />,
      color: "from-[#3498db] to-[#2980b9]",
      bgColor: "bg-[#3498db]/20",
      trend: totalShares > 0 ? "up" : "neutral",
    },
    {
      title: "Unique Files",
      value: uniqueFiles,
      change: `${uniqueFiles} total files`,
      icon: <FileText className="w-6 h-6" />,
      color: "from-[#2ecc71] to-[#27ae60]",
      bgColor: "bg-[#2ecc71]/20",
      trend: "neutral",
      ...
      {error && (
        <div className="mb-4 flex items-start gap-3 rounded-lg border border-red-700 bg-red-900/30 px-4 py-3 text-sm text-red-300">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold mb-0.5">Could not reach the analytics API</p>
            <p className="break-all opacity-80">{error}</p>
            <p className="mt-1 opacity-60 text-xs">
              Check: (1) backend is running, (2) Vite proxy is configured for <code>/api</code>,
              (3) CORS is enabled on the Express server.
            </p>
          </div>
          <button
            onClick={() => loadData(timeRange)}
            className="shrink-0 rounded bg-red-700 hover:bg-red-600 px-2 py-1 text-xs font-medium text-white"
          >
            Retry
          </button>
        </div>
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-400">
              Real insights from {uniqueFiles} tracked files •{" "}
              {totalShares} total link copies
            </p>
          </div>
        ...
        <div className="bg-white/80 dark:bg-gray-800/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          {analyticsData.dailyShares.length > 0 ? (
            ...
            <div className="h-72 flex flex-col items-center justify-center">
              <Share2 className="w-12 h-12 text-gray-600 mb-4" />
              <p className="text-gray-500">No sharing data available yet</p>
              <p className="text-gray-600 text-sm">Share some files to see analytics</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white/80 dark:bg-gray-800/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
          ...
          {analyticsData.topFiles.length > 0 ? (
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analyticsData.topFiles}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend />
                  <Bar dataKey="shares" name="Link Copies" fill="#3498db" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="downloads" name="Downloads" fill="#2ecc71" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="h-72 flex flex-col items-center justify-center">
              <FileText className="w-12 h-12 text-gray-600 mb-4" />
              <p className="text-gray-500">No file sharing activity yet</p>
              <p className="text-gray-600 text-sm">Files will appear here when shared</p>
            </div>
          )}
        </div>
      </div>
